...
public class CalculateAverage_PEWorkshop7 {
    ...
    /**
     * Avoid unnecessary creation of String objects for location name unless needed
     *
     * In the Row objects we can store the bytes. The hash table can compute the hash code of 
     * the location bytes and only if no entry exists in the table for that location, it
     * needs to copy the bytes when creating a new Row object.
     *
     */
    ...
    private static class Table {
        ...
        public void put(byte[] locationBytes, int locationBytesLength, int temperature) {
            int hashCode = hashCode(locationBytes, locationBytesLength);
            ...
            if (r.hashCode == hashCode && Arrays.equals(r.locationNameBytes, 0, r.locationNameBytes.length - 1,
                    locationBytes, 0, locationBytesLength - 1)) {
            ...
            Row nextRow = r.next;
            while (nextRow != null) {
                if (nextRow.hashCode == hashCode && Arrays.equals(nextRow.locationNameBytes, 0,
                        nextRow.locationNameBytes.length - 1, locationBytes, 0, locationBytesLength - 1)) {
    ...
    private static void processLine(byte[] locationBytes, int locationBytesLength, byte[] temperatureBytes,
        ...
        records.put(locationBytes, locationBytesLength, temperature);
    }
