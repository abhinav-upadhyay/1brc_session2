...
public class CalculateAverage_PEWorkshop9 {
    ...
    private static class Table {
        ...
        public void put(long locationStartOffset, int locationLength, int hashCode, int temperature) {
            int index = hashCode & TABLE_MASK;
            Row r = table[index];
            if (r == null) {
                byte[] locationBytesCopy = new byte[locationLength];
                for (int i = 0; i < locationLength; i++) {
                    locationBytesCopy[i] = UNSAFE.getByte(locationStartOffset + i);
                }
            ...
            if (r.hashCode == hashCode) {
                ...
                for (int i = 0; i < locationLength; i++) {
                    if (r.locationNameBytes[i] != UNSAFE.getByte(locationStartOffset + i)) {
                        isCollision = true;
                        break;
                    }
                }
            ...
            Row nextRow = r.next;
            while (nextRow != null) {
                if (nextRow.hashCode == hashCode) {
                    boolean isCollision = false;
                    for (int i = 0; i < locationLength; i++) {
                        if (nextRow.locationNameBytes[i] != UNSAFE.getByte(locationStartOffset + i)) {
                            isCollision = true;
                            break;
                        }
                    }
    ...
    private static void processLine(long locationStartOffset, int locationLength, int hashCode, byte[] temperatureBytes,
            int temperatureBytesLength, Table records) {
        int temperature = parseTemperature(temperatureBytes, temperatureBytesLength);
        records.put(locationStartOffset, locationLength, hashCode, temperature);
    }
    ...
    private static Table readFile(long startAddress, long endAddress) {
        ...
        byte b;
        int hashCode = 1;
        long locationStartOffset = currentOffset;
        ...
        while (currentOffset < endAddress) {
            ...
            processLine(locationStartOffset, locationLength, hashCode, temperatureBytes, temperatureArrayOffset, table);
            locationStartOffset = currentOffset;
            locationLength = 0;
